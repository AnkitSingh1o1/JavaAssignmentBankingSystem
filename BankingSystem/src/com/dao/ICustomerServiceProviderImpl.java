...
public class ICustomerServiceProviderImpl implements ICustomerServiceProvider{
	...
	@Override
	public int deposit(long accountNumber, double amount) throws SQLException, InvalidAccountException {
		Connection con = DBConnection.dbConnect();
		...
		// prepare statement
		PreparedStatement pstmt = con.prepareStatement(sql);
		pstmt.setDouble(1, (amount+currAmount));
	...
	public int withdraw(long accountNumber, float amount)
		...
		if(account.getAccount_type().equalsIgnoreCase("current"))
		...
		// prepare statement
		PreparedStatement pstmt = con.prepareStatement(sql);
		pstmt.setDouble(1, (currAmount-amount));
		pstmt.setDouble(2, accountNumber);
	...
	public AccountAndCustomer getAccountDetails(long accountNumber) throws SQLException {
		...
		Connection con = DBConnection.dbConnect();
		String sql = "select * from account a join customer c on "
				+ "a.customer_id = c.customer_id where account_id = ?";
